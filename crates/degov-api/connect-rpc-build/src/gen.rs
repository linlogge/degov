        ...
        if method.server_streaming {
            quote! {
                pub fn #method_name<T, H, S>(
                    handler: H
                ) -> impl FnOnce(axum::Router<S>) -> connect_rpc::router::RpcRouter<S>
                where
                    H: connect_rpc::handler::RpcHandlerStream<#input_type, #output_type, T, S>,
                    T: 'static,
            ...
            quote! {
                ...
                where
                    H: connect_rpc::handler::RpcHandlerUnary<#input_type, #output_type, T, S>,
                    T: 'static,
                    S: Clone + Send + Sync + 'static,
                {
                    move |router: axum::Router<S>| {
